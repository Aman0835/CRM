...
export default function PayrollDetail() {
      ...
      if (payRes.success && payRes.data) {
        ...
        if (empRes.success) {
          const emp = empRes.data.find(
            (e) => e.employeeId === payRes.data.employeeId,
          );
          if (emp) {
            setEmployee(emp);
          } else {
  ...
  const leaveDeduction = Math.min(
    payroll.deductions,
    Math.max(0, payroll.deductions - pf - pt),
  );
  ...
  const handlePrintOrPDF = () => {
    ...
    doc.write(`
            <html>
                <head>
                    <title>Payslip - ${fallbackEmployee.firstName} ${fallbackEmployee.lastName}</title>
                    <style>
                        ...
                        <table class="info-grid">
                            <tr>
                                <td>
                                    ...
                                    <span class="info-value">${fallbackEmployee.firstName} ${fallbackEmployee.lastName}</span>
                                </td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <span class="info-value" style="text-transform: capitalize;">${fallbackEmployee.role}</span>
                                </td>
                                <td>
  ...
  const handleDownloadPDF = () => {
    const exporter =
      typeof html2pdf === "function" ? html2pdf : html2pdf && html2pdf.default;
    if (!exporter) {
            ...
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; padding: 40px; color: #334155; background-color: #ffffff; width: 720px; font-size: 12px; line-height: 1.5;">
                ...
                <div style="background-color: #f8fafc; padding: 15px; border-radius: 12px; border: 1px solid #e2e8f0; margin-bottom: 25px; display: flex; justify-content: space-between; flex-direction: row;">
                    <div style="width: 25%; float: left;">
                        ...
                        <strong style="font-size: 11px; color: #1e293b;">${fallbackEmployee.firstName} ${fallbackEmployee.lastName}</strong>
                    </div>
                    <div style="width: 25%; float: left;">
                    ...
                    <div style="width: 25%; float: left;">
                        ...
                        <strong style="font-size: 11px; color: #1e293b; text-transform: capitalize;">${fallbackEmployee.role}</strong>
                    </div>
    ...
    const opt = {
      margin: 10,
      filename: `Payslip_${fallbackEmployee.firstName}_${fallbackEmployee.lastName}_Month_${payroll.month}_${payroll.year}.pdf`,
  ...
  return (
    <DashboardLayout
      ...
      subtitle={`Monthly Wage Ledger for ${fallbackEmployee.firstName} ${fallbackEmployee.lastName}`}>
      {/* Back Button */}
      <button
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            <EmployeeInfoCard employee={fallbackEmployee} payroll={payroll} />
    ...
    <div className="panel-surface rounded-3xl p-6 shadow-sm flex flex-col items-center text-center gap-4">
      {employee.profileImage ? (
        <img
          ...
          className="h-20 w-20 rounded-2xl object-cover border-2 border-blue-500/10 shadow-sm"
        />
      ) : (
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold mt-0.5 capitalize">
          {employee.role}
        </p>
        <span
          className={`inline-block px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase mt-2 border ${statusColors[payroll.status] || statusColors.draft}`}>
          {payroll.status}
        </span>
      </div>
      ...
      <div className="w-full border-t border-slate-100 dark:border-slate-800 pt-4 text-xs text-left space-y-2.5">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-semibold text-slate-800 dark:text-slate-200 capitalize">
            {employee.role === "manager" ? "Administration" : "Styling Chair"}
          </span>
        </div>
        <div className="flex justify-between">
  ...
  return (
    <div className="panel-surface rounded-3xl p-6 shadow-sm space-y-5">
      <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3">
        ...
        <span className="text-xs font-semibold text-blue-600 bg-blue-50 dark:bg-blue-950/30 dark:text-blue-450 px-2.5 py-0.5 rounded-lg">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        {metricItem(
          "Leave Days",
          leaveDays,
          leaveDays > 0 ? "text-orange-500" : "text-slate-400",
        )}
        {metricItem(
          "Overtime Hours",
          `${overtimeHours} hrs`,
      ...
      <div className="border-t border-slate-100 dark:border-slate-800 pt-4 space-y-3 text-xs">
        ...
        <div className="flex justify-between items-center border-t border-slate-100 dark:border-slate-800 pt-3 text-sm">
          ...
          <strong className="text-blue-600 dark:text-blue-450 text-base">
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-semibold text-slate-700 dark:text-slate-300 font-mono">
            TXN$${payroll._id?.slice(-8).toUpperCase()}
          </span>
        </div>
        <div className="flex justify-between">
...
function EarningsTable({ basic, hra, medical, travel, overtime, otherAllow }) {
  ...
  return (
    <div className="space-y-3">
      <div className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-wider border-b border-slate-100 dark:border-slate-800 pb-1.5">
        Earnings
      </div>
      <table className="w-full text-xs">
        <tbody>
          {rows.map((r) => (
            <tr
              key={r.label}
              className="border-b border-slate-50 dark:border-slate-900/50 last:border-0">
              <td className="py-2 text-slate-500 dark:text-slate-400">
                {r.label}
              </td>
              <td
                className={`py-2 text-right font-semibold ${r.plus ? "text-emerald-600 dark:text-emerald-500" : "text-slate-700 dark:text-slate-300"}`}>
...
function SalarySlipCard({
  ...
  return (
    <div
      id="salary-slip-card"
      className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-sm space-y-6 max-w-[800px] mx-auto text-xs text-slate-800 dark:text-slate-100">
      <div className="flex justify-between items-start border-b border-slate-100 dark:border-slate-800 pb-5">
        <div>
          <h4 className="text-lg font-black text-slate-850 dark:text-slate-55 tracking-tight">
            Diva
          </h4>
          <p className="text-[9px] uppercase font-bold text-slate-400 tracking-widest mt-0.5">
            The Salon
          </p>
          <p className="text-[9px] text-slate-400 mt-1.5 leading-relaxed font-medium">
            12, Connaught Place Outer Circle
            <br />
            New Delhi, DL 110001
          </p>
        </div>
        <div className="text-right">
          <h5 className="text-base font-bold text-slate-850 dark:text-slate-100 uppercase tracking-wider">
            Salary Slip
          </h5>
          <p className="text-xs text-blue-600 dark:text-blue-450 font-bold mt-1">
            Month {payroll.month} / {payroll.year}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50 dark:bg-slate-950/20 p-5 rounded-2xl border border-slate-100 dark:border-slate-800">
        ...
        <div>
          ...
          <span className="font-bold text-slate-800 dark:text-slate-200 block mt-0.5 capitalize">
            {employee.role}
          </span>
        </div>
        <div>
      ...
      <div className="border-t border-slate-100 dark:border-slate-800 pt-4 flex flex-col md:flex-row md:justify-between items-end gap-4">
        <div className="text-[10px] text-slate-400 leading-relaxed max-w-[320px]">
          <div className="flex justify-between py-0.5">
            ...
            <strong className="text-slate-650 dark:text-slate-300 font-semibold ml-2">
              Bank Transfer (NEFT)
            </strong>
          </div>
          <div className="flex justify-between py-0.5">
            <span>Transaction ID:</span>
            <strong className="text-slate-650 dark:text-slate-300 font-mono ml-2">
              TXN${payroll._id?.slice(-8).toUpperCase()}
            </strong>
          </div>
        </div>
