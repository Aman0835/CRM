...
import { FiPlus, FiFileText, FiTrash2, FiSearch, FiX } from "react-icons/fi";
...
export default function Payroll() {
    ...
    return (
        <DashboardLayout
            ...
            action={
                <button onClick={() => setIsGenModalOpen(true)} type="button"
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between border-b border-slate-100 dark:border-slate-800 pb-4 mb-4">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                        <div className="flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs text-slate-400 focus-within:border-blue-500 focus-within:bg-white transition-all dark:bg-slate-800 dark:border-slate-700 dark:focus-within:bg-slate-855">
                            <FiSearch />
                            <input type="text" placeholder="Search barber..." value={search} onChange={(e) => setSearch(e.target.value)}
                                ...
                                {filteredPayrolls.map((payroll) => {
                                    ...
                                    return (
                                        <tr key={payroll._id} className="border-b border-slate-100 dark:border-slate-800/60 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-850/10 transition-colors">
                                            ...
                                            <td className="py-3.5 px-4 text-slate-500 dark:text-slate-400 font-medium">{payroll.month}/{payroll.year}</td>
                                            <td className="py-3.5 px-4 text-slate-500 dark:text-slate-400">${payroll.baseSalary.toLocaleString()}</td>
                                            <td className="py-3.5 px-4 text-emerald-600 dark:text-emerald-500 font-medium">+${payroll.overtimeAmount.toLocaleString()}</td>
                                            <td className="py-3.5 px-4 text-red-500 font-medium">-${payroll.deductions.toLocaleString()}</td>
                                            <td className="py-3.5 px-4 font-bold text-blue-600 dark:text-blue-400">${payroll.netSalary.toLocaleString()}</td>
                                            <td className="py-3.5 px-4">
                                                <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase ${
                                                    payroll.status === "paid"
                                                        ? "bg-emerald-50 text-emerald-600 border border-emerald-100 dark:bg-emerald-950/20 dark:text-emerald-450 dark:border-emerald-900"
                                                        : "bg-orange-50 text-orange-600 border border-orange-100 dark:bg-orange-950/20 dark:text-orange-455 dark:border-orange-900"
                                                }`}>
                                                    {payroll.status}
                                                </span>
                                            </td>
                                            <td className="py-3.5 px-4 text-center flex items-center justify-center gap-2">
                                                <button onClick={() => navigate(`/payroll/${payroll._id}`)}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-xs">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-[380px] rounded-[24px] border border-slate-200 dark:border-slate-800 p-6 shadow-2xl text-slate-800 dark:text-slate-105">
                        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                            ...
                            <button onClick={() => setIsGenModalOpen(false)}
                        ...
                        <form onSubmit={handleGenerate} className="mt-4 space-y-4 text-xs">
                            <div>
                                ...
                                <select value={genMonth} onChange={(e) => setGenMonth(e.target.value)} required
                            ...
                            <div>
                                ...
                                <select value={genYear} onChange={(e) => setGenYear(e.target.value)} required
                                    className="w-full h-10 mt-1.5 rounded-xl border border-slate-200 bg-slate-50 px-3 text-slate-600 outline-none focus:border-blue-500 focus:bg-white dark:bg-slate-800 dark:border-slate-700 dark:text-slate-350 dark:focus:bg-slate-855">
                                    <option value="2025">2025</option>
                                    <option value="2026">2026</option>
                                </select>
