...
export default function Dashboard() {
  ...
  const metrics = [
    ...
    },
    {
      ...
      icon: FiDollarSign,
      iconBg:
        "bg-purple-50 text-purple-600 border border-purple-100 dark:bg-purple-950/20 dark:text-purple-400 dark:border-purple-900/30",
      path: "/payroll",
    ...
    },
    {
      ...
      value:
        presentCount > 0 && totalEmployees > 0
      ...
      icon: FiTrendingUp,
      iconBg:
        "bg-cyan-50 text-cyan-600 border border-cyan-100 dark:bg-cyan-950/20 dark:text-cyan-400 dark:border-cyan-900/30",
      path: "/attendance",
  ...
  const getWeeklyAttendanceData = () => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const present = dayLogs.filter(
        (log) => log.status === "Present" || log.status === "Half Day",
      ).length;
    ...
    leavesList.forEach((l) => {
      if (l.status === "approved") {
        ...
        if (
          ...
          reasonText.includes("medical")
        ) {
          sick += duration;
        } else if (
          reasonText.includes("annual") ||
          reasonText.includes("vacation") ||
          reasonText.includes("trip")
        ) {
          annual += duration;
  ...
  const totalLeavesInChart = leaveTypesData.reduce(
  ...
  const getTopBarbers = () => {
    const list = employees
      .map((emp) => {
        ...
        const presentLogs = empLogs.filter(
          (l) => l.status === "Present" || l.status === "Half Day",
        ).length;
        const rate = empLogs.length > 0 ? presentLogs / empLogs.length : 0;
        const ratingScore =
          empLogs.length > 0 ? (4.5 + rate * 0.5).toFixed(1) : "0.0";
        return {
          name: `${emp.firstName} ${emp.lastName}`,
          specialty:
            emp.role === "manager" ? "Shop Manager" : "Fade Specialist",
          rating: ratingScore,
  ...
  const getTodayShifts = () => {
    ...
    employees.forEach((emp) => {
      ...
      if (log && log.checkIn) {
        list.push({
          time: new Date(log.checkIn).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
            ...
            {metrics.map((metric) => {
              ...
              return (
                <div
                  ...
                  {/* Ambient Glow */}
                  <div
                    className={`absolute top-0 right-0 h-28 w-28 translate-x-10 -translate-y-10 rounded-full ${metric.glowColor} blur-xl transition-all duration-500 group-hover:scale-110`}></div>
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div
                      className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-transform duration-300 group-hover:scale-105 relative z-10 ${metric.iconBg}`}>
              ...
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4 mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                    This week · 6 employees
                  </p>
                </div>
                <span className="text-xs font-semibold text-slate-500 bg-slate-100 dark:bg-slate-800 dark:text-slate-400 px-2.5 py-1 rounded-lg">
                  Jul 2025
                </span>
              </div>
                ...
                {leaveTypesData.map((type) => (
                  <div
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-slate-600 dark:text-slate-405 font-medium">
                        {type.name}
                      </span>
                    </div>
                ...
                {upcomingAppointments.map((appointment, i) => (
                  <div
                    ...
                    <div className="text-right text-xs">
                      ...
                      <span className="text-slate-450 dark:text-slate-500 mt-0.5 block font-bold uppercase text-[9px] tracking-wide">
                        {appointment.barber}
                      </span>
                    </div>
                  </div>
